  ...
  public function data_budget()
  {
    ...
    if($number > 0)
    {
      ...
      for($i=0; $i<$number; $i++)
      {
        ...
        $insert->price_per = round(trim($_POST["price_per"][$i]),2);
        $insert->unit = trim($_POST["unit"][$i]);
        $insert->unitsap = trim($_POST["unitsap"][$i]);
        $insert->total = round(trim($_POST["total"][$i]),2);
...
// dd($request->list_old[2]);
    if($number > 0){
      ...
      for($j=0 ; $j<$number ; $j++){
        if($request->list_old[$j] == null){
          ...
          $update->price_per = round(trim($request->price_per_old[$j]),2);
          $update->unit = trim($request->unit_old[$j]);
          $update->unitsap = trim($request->unitsap_old[$j]);
          $update->total = round(trim($request->total_old[$j]),2);
    ...
    if(!is_null($request->list)){
      ...
      for($i=0; $i<count($request->list); $i++)
      {
        ...
        $insert->price_per = round(trim($request->price_per[$i]),2);
        $insert->unit = trim($request->unit[$i]);
        $insert->unitsap = trim($request->unitsap[$i]);
        $insert->total = round(trim($request->total[$i]),2);
     ...
     if($data->count() > 0)
     {
      ...
      if(!empty($insert_data))
      {
        ...
        for($j = 0; $j < count($insert_data); $j++ ){
          ...
          $insert->price_per = round($insert_data[$j++]['price_per'],2);
          $insert->unit = $insert_data[$j++]['unit'];
          $insert->unitsap = $insert_data[$j++]['unitsap'];
          $insert->total = round($insert_data[$j++]['total'],2);
